...
# تم تحديثها لاستخدام الـ API الرسمي لموقع iVasms
# ======================
...
IVASMS_API = {
    ...
    "api_base": "https://www.ivasms.com/api",
    "messages_endpoint": "https://www.ivasms.com/api/sms/received",
    "username": "svdmggr@hi2.in", # امسح ده واكتب بريدك الحقيقي
    "password": "FirstNameAhmed1#", # امسح ده واكتب باسوردك الحقيقي
    "session": None,
    "is_logged_in": False,
    "cookies": None,
    "csrf_token": None,
...
# ======================
# 🔧 إعدادات عامة (حط التوكن في ملف .env فقط)
# ======================
USERNAME = IVASMS_API["username"]
PASSWORD = IVASMS_API["password"]
BOT_TOKEN = os.getenv("BOT_TOKEN")  # اقرأ التوكن من ملف .env

if not BOT_TOKEN:
    raise SystemExit("❌ BOT_TOKEN must be set in .env file (BOT_TOKEN=xxxx)")

CHAT_IDS = [
    "-1002261379064", # ايدي القروب البيترسل فيهو الكود
]
REFRESH_INTERVAL = 3
TIMEOUT = 100
MAX_RETRIES = 5
...
ADMIN_IDS = [6264668799] # حط الايدي حقك هنا
DB_PATH = "bot.db"
...
# ======================
# 🌍 رموز الدول (نفسها ما غيرنا فيها حاجة)
# ======================
COUNTRY_CODES = {
    "1": ("USA/Canada", "🇺🇸", "US"),
    "7": ("Russia", "🇷🇺", "RU"),
...
# ======================
# 📡 دوال الاتصال بلوحة iVasms (بعد التصحيح)
# ======================

def login_to_ivasms():
    """تسجيل الدخول إلى لوحة iVasms الرسمية باستخدام API"""
    scraper = cloudscraper.create_scraper()
    login_url = IVASMS_API["login_url"]
    username = IVASMS_API["username"]
    password = IVASMS_API["password"]
    ...
    try:
        print(f"[{IVASMS_API['name']}] محاولة تسجيل الدخول...")
        response = scraper.post(login_url, data=payload, timeout=30)
        
        if response.status_code == 200 and "dashboard" in response.url.lower():
            IVASMS_API["session"] = scraper
            IVASMS_API["is_logged_in"] = True
            print(f"[{IVASMS_API['name']}] ✅ تسجيل الدخول ناجح")
            return True
        else:
            print(f"[{IVASMS_API['name']}] ❌ فشل تسجيل الدخول - كود {response.status_code}")
            return False
    except Exception as e:
        print(f"[{IVASMS_API['name']}] ❌ خطأ في تسجيل الدخول: {e}")
        return False

def fetch_ivasms_messages():
    """جلب أحدث رسائل SMS من iVasms باستخدام API بسيط"""
    if not IVASMS_API.get("is_logged_in", False):
        if not login_to_ivasms():
            return []
    
    scraper = IVASMS_API["session"]
    messages_url = "https://www.ivasms.com/api/sms/received?limit=50"
    
    try:
        response = scraper.get(messages_url, timeout=30)
        if response.status_code == 200:
            ...
            for msg in data.get("data", []):
                messages.append({
                    ...
                    "number": clean_number(msg.get("number", "")),
                    "sms": msg.get("message", "")
                })
            return messages
        else:
            print(f"[!] فشل جلب الرسائل: {response.status_code} - {response.text}")
            IVASMS_API["is_logged_in"] = False  # جلسة منتهية
            return []
    except Exception as e:
